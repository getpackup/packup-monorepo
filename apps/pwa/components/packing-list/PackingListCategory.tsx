...
import { CollapsibleBox, FlexContainer, PackingListAddItem, PackingListItem } from '../index'
...
export const PackingListCategory: FunctionComponent<PackingListCategoryProps> = ({
  categoryName,
  sortedItems,
  tripId,
  trip,
          ...
          {sortedItems && sortedItems.length > 0 ? (
            <>
              {sortedItems.map((item, idx) => (
                <PackingListItem
                  key={item.id}
                  isFirstCategoryAndItem={categoryIndex === 0 && idx === 0}
                  tripId={tripId}
                  item={item}
